...
- 이 내용이 전 단계에서 어떻게 넘어왔으며, 후속 단계(설계/시공/인허가 등)에 어떤 영향을 미치는지 연결
...
# =========================================================
    ...
    with col_kpi1:
        st.metric("분석 대상 파일", f"{len(uploaded_files)}개" if uploaded_files else "기본 가상 데이터")
    with col_kpi2:
    ...
    # 동적 마크다운 보고서 생성
    raw_texts_summary = "\n".join([f"- {txt[:100]}..." for txt in raw_text_corpus]) if raw_text_corpus else "업로드된 비정형 메모 없음 (기본 데이터 활용)"
